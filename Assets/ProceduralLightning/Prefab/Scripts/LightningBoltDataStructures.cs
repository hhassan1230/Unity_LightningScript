...
namespace DigitalRuby.ThunderAndLightning
{
    ...
    public sealed class LightningBoltParameters
    {
        ...
        /// <summary>
        /// Used to generate random numbers, most NOT be null. Passing a random with the same seed and parameters will result in the same lightning.
        /// </summary>
...
// Unity 4.X does not support glow or fade or elapsed time
        ...
        public void Cleanup(bool willCache)
        {
            ...
            {
                Debug.LogError("Cleanup called twice OR called while lightning generation taking place");
            }
        ...
        private void RenderGroup(LightningBoltSegmentGroup group, LightningBoltParameters p)
        {
            ...
            currentLineRenderer.BeginLine(group.Segments[group.StartIndex].Start, group.Segments[group.StartIndex].End, radius, group.Color, fadeLifeTime, p.GlowWidthMultiplier, p.GlowIntensity);
            for (int i = group.StartIndex + 1; i < group.Segments.Count; i++)
            {
                ...
                currentLineRenderer.AppendLine(group.Segments[i].Start, group.Segments[i].End, radius, group.Color, fadeLifeTime, p.GlowWidthMultiplier, p.GlowIntensity);
            }
        }
